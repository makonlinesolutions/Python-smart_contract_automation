...
from utils.env_loader import RPC_URL, LIQUIDITY_AMOUNT, TOKEN_IN_ADDRESS, TOKEN_OUT_ADDRESS, SWAP_ROUTER_ADDRESS, GAS_FEE
import json

with open("abis/router_abi.json") as f:
    router_abi = json.load(f)

def run_liquidity(wallet):
    print(f"\n💧 Adding liquidity for wallet: {wallet['address']}")
    ...
    txn = router.functions.addLiquidity(
        ...
        w3.to_wei(LIQUIDITY_AMOUNT, 'ether'),
        w3.to_wei(LIQUIDITY_AMOUNT, 'ether'),
        0,
        0,
        acct.address,
    ...
    print("✅ Liquidity Tx:", tx_hash.hex())